...
public class ApproveStudentProjectParticipantHandler : IRequestHandler<ApproveStudentProjectParticipantCommand>
{
    ...
    private readonly ILogger<ApproveStudentProjectParticipantHandler> _logger;
    ...
    public ApproveStudentProjectParticipantHandler(
        ...
        ILogger<ApproveStudentProjectParticipantHandler> logger)
    {
    ...
    private async Task SendNotificationAsync(long userId, string projectTitle, CancellationToken cancellationToken)
    {
        try
        {
            var message = $"Ваша заявка на участие в проекте \"{projectTitle}\" была одобрена.";
        ...
        {
            _logger.LogError(exception, "Failed to send approval notification to user {UserId} for project \"{ProjectTitle}\"", userId, projectTitle);
        }
    }
